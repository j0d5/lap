...
async fn get_still_image_thumbnail_with_ffmpeg(
    file_path: &str,
    thumbnail_size: u32,
    square_pad: bool,
) -> Result<Option<Vec<u8>>, String> {
    ...
    // Keep the full frame: fit inside box, no crop. Optionally pad to a square canvas for grid thumbnails.
    let filter = if square_pad {
        format!(
    ...
    .await
    {
        ...
        Err(err) => {
            eprintln!(
                "FFmpeg still-image thumbnail failure for {}. Err: {}",
                file_path, err
            );
            Ok(None)
        }
...
#[cfg(not(target_os = "macos"))]
pub async fn get_still_image_thumbnail(
    file_path: &str,
    thumbnail_size: u32,
    square_pad: bool,
) -> Result<Option<Vec<u8>>, String> {
    get_still_image_thumbnail_with_ffmpeg(file_path, thumbnail_size, square_pad).await
}

#[cfg(not(target_os = "macos"))]
pub fn get_still_image_thumbnail_sync(
    file_path: &str,
    thumbnail_size: u32,
    square_pad: bool,
) -> Result<Option<Vec<u8>>, String> {
    let handle = tokio::runtime::Handle::try_current().map_err(|_| "No runtime handle")?;
    tokio::task::block_in_place(|| {
        handle.block_on(get_still_image_thumbnail(
            file_path,
            thumbnail_size,
            square_pad,
        ))
